...
{
	if(update_flag) // depending upon what needs to be changed on the basis of the flag
	{
		int0_count++;
		if(int0_count==2)
		{
...
void isr_t1() interrupt 3timer 1 interrupt // is to increase or decrease the speed of motor
{	
	{
		if(exp_rpmcurr_rpm)
		{
...
-----------------------------------------------
	IT0 = 1;    Configure interrupt 0 for falling edge on INT0 (P3.2) // 8051 configuration
...
void init()           //all initializing functions for keypad, display & timers
{
	lcd_init();
	init_display();
	init_keypad();
...
void init_keypad()       //initialize keypad pins - key ready is initially 0 since we havent given any input
{
	row1=1;
	row2=1;
	row3=1;
...
void init_timer0() //initialize or declare timer 1 of microcontroller (configuring the 8051)
{
	TMOD=0x01;timer 1 mode 1 selected -16 bit timer mode
	TL0=0x00;load timer count
	TH0=0x00;load timer count
	TR0=1;start timer // starts the timer - in timer mode internal cycle

}

void init_timer1()   //initialize or declare timer 2 of microcontroller 
{
	TMOD=0x10;timer 1 mode 0 selected-   hardware enabled startstop active // in counter mode count  is increased
...
void lcd_data(unsigned char data1)    //setting values for lcd pins, rs being 1 for data mode - read/write
	{
		RS=1;
		RW=0;
		LCDPort=data1;
...
void lcd_init()              //initializing lcd display
	{ 
		lcd_cmd(0x01);
		delay(10);
		lcd_cmd(0x06);
...
{
	switch(scan_no) //scans the number, which digit it is
	{
		case 0	col1=0;
				col2=1;
				col3=1;
...
void k()    //depending on key_ready returns value of entered digits (k has 2 variables, key ready either 0 or 1 which returns the  value in ASCII)
{
	if(key_ready==0) // if key ready 0, it wont go in 450 (will be outside the loop)
	{
		if(dcnt==25)
		{
			if(chk==0)
	...
	else if(key_ready==1) // if key ready is 1 it will go in loop & map the value betn keypad & lcd so that it compares the value & adjusts the rpm)
	{
		if(chk==0)
		{
			krcount=24;
...
void display()  //updates the display (just to display the 4 digits)
{
...
{
	update_flag=0; // result updates  the flag then it displays the actual value of the speed on LCD
	ds1=curr_rpm%10;
	ds2=(curr_rpm10)%10; // %10 either increase or decrease update the rpm value to match the exxpected value
	ds3=(curr_rpm100)%10;
	ds4=((curr_rpm1000)%10);
	update_flag=1;
	ds4=ascii_tab2[ds3];
...
void measure_rpm() //measures speed in rpm , increase the count between 0-100%
{
...
void pwm()  //expected speed in rpm
{
	if(rpm_update==1)
	{
		exp_rpm=(((key_code1-0x30)1000)+((key_code2-0x30)100)+((key_code3-0x30)10)+(key_code4-0x30)); //is to update rpm which just displays 4 digit
		rpm_update=0;
	}
